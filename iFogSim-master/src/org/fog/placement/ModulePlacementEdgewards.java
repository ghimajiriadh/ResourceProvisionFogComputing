...
public class ModulePlacementEdgewards extends ModulePlacement {
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		for (Integer deviceId : path) {
			...
			while (modulesToPlace.size() > 0) { // Loop runs until all modules in modulesToPlace are deployed in the
												// path
				...
				if (upsteamDeviceId > 0) {
					...
					if (totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()) {
						...
						System.out.println("Placement of operator " + moduleName + " on device " + device.getName()
								+ " successful.");
	 ...
	 * 
	 * @param module
	 *            the module that needs to be shifted northwards
	 * @param deviceId
	 *            the fog device ID that it is currently on
