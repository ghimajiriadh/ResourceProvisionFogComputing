 ...
 *
 */
public class ChangeFogDeviceDialog extends JDialog {
	...
	public ChangeFogDeviceDialog(FogDevice fogDevice, final Graph graph, final JFrame frame) {
		this.graph = graph;
	...
	private JPanel createInputPanelArea() {
		...
		deviceName.setText(fogDevice.getName());
		...
		level.setText(String.valueOf(fogDevice.getLevel()));
		...
		upBw = new JTextField();
		upBw.setText(String.valueOf(fogDevice.getUplinkBandwidth()));
		...
		downBw.setText(String.valueOf(fogDevice.getDownlinkBandwidth()));
		...
		mips = new JTextField();
		mips.setText(String.valueOf(fogDevice.getHost().getTotalMips()));
		...
		ram = new JTextField();
		ram.setText(String.valueOf(fogDevice.getHost().getRam()));
		...
		rate = new JTextField();
		rate.setText(String.valueOf(fogDevice.getRatePerMips()));
		rateLabel.setLabelFor(rate);
		springPanel.add(rate);

		// Lay out the panel.
		SpringUtilities.makeCompactGrid(springPanel, 7, 2, // rows, cols
				6, 6, // initX, initY
				6, 6); // xPad, yPad
		// updatePanel("core");
		return springPanel;
	...
	private void prompt(String msg, String type) {
		JOptionPane.showMessageDialog(ChangeFogDeviceDialog.this, msg, type, JOptionPane.ERROR_MESSAGE);
	}
}
