 ...
 *
 */
public class ChangeSensorDialog extends JDialog {
	 ...
	 *            the parent frame
	 */
	public ChangeSensorDialog(Sensor sensor, final Graph graph, final JFrame frame) {
		this.graph = graph;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (sensorName.getText() == null || sensorName.getText().length() < 1) {
					...
					if (dist.equals("Normal")) {
						...
						if (!catchedError) {
							SensorGui sensor = new SensorGui(sensorName.getText().toString(), _sensorType,
									(String) distribution.getSelectedItem(), normalMean_, normalStdDev_, uniformLow_,
	...
	private JPanel createInputPanelArea() {
		...
		sensorType.setText(sensor.getTupleType());
		...
		// Lay out the panel.
		SpringUtilities.makeCompactGrid(springPanel, 8, 2, // rows, columns
				6, 6, // initX, initY
				6, 6); // xPad, yPad
		return springPanel;
	}
	...
	private void prompt(String msg, String type) {
		JOptionPane.showMessageDialog(ChangeSensorDialog.this, msg, type, JOptionPane.ERROR_MESSAGE);
	}
}
