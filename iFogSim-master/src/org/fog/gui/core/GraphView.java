...
public class GraphView extends JPanel {
	...
	private Map<String, Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>> buttonMap;

	public GraphView(final Graph graph) {

		this.graph = graph;
		imgHostInactive = Toolkit.getDefaultToolkit().getImage(this.getClass().getResource("/images/host.png"));
		...
		imgActuatorModule = Toolkit.getDefaultToolkit()
				.getImage(this.getClass().getResource("/images/actuatorModule.png"));
		...
		for (Node node : graph.getAdjacencyList().keySet()) {
			...
			for (Edge edge : edgeList) {
				...
				if (node.getType().equals("SENSOR") || node.getType().equals("ACTUATOR")) {
					...
					Node child = (((FogDeviceGui) node).getLevel() > ((FogDeviceGui) neighbour).getLevel()) ? node
							: neighbour;
					Node parent = (((FogDeviceGui) node).getLevel() < ((FogDeviceGui) neighbour).getLevel()) ? node
							: neighbour;
	...
	private void initComponents() {
		...
		canvas = new JPanel() {
			...
			public void paint(Graphics g) {
				...
				/*
				 * int offsetX = canvas.getWidth() / 2; int offsetY = canvas.getHeight() / 2;
				 */
				// System.out.println("sys:" + canvas.getWidth() + ":" + canvas.getHeight());

				int height = 40;
				/*
				 * double angle = 2 * Math.PI / graph.getAdjacencyList().keySet().size(); int
				 * radius = offsetY / 2 - 20;
				 */
				...
				coordForNodes = getCoordForNodes(levelToPlaceHolderMap, endpointPlaceHolders, levelMap, endpoints,
						minLevel, maxLevel);
				...
				for (Entry<Node, Coordinates> entry : coordForNodes.entrySet()) {
					...
					// int nodeWidth = Math.max(width, f.stringWidth(entry.getKey().getNodeText()) +
					// width / 2);
					...
					case "host":
						g.drawImage(imgHostInactive, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);

						break;
					case "APP_MODULE":
						g.drawImage(imgAppModule, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);
					...
					case "core":
					case "edge":
						g.drawImage(imgSwitch, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);
						break;
					case "FOG_DEVICE":
						g.drawImage(imgHostInactive, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
						...
						g.drawString(nodeName, wrapper.getX() - f.stringWidth(nodeName) / 2,
								wrapper.getY() + nodeHeight);
						if (!buttonMap.containsKey(entry.getKey().getName())) {
							Pair<Integer, Integer> coordinate = new Pair<>(wrapper.getX() - nodeWidth / 2,
									wrapper.getY() - nodeHeight / 2);
							Pair<Integer, Integer> widthWeight = new Pair<>(nodeWidth + wrapper.getX() - nodeWidth / 2,
									nodeHeight + wrapper.getY() - nodeHeight / 2);
							Pair<Pair<Integer, Integer>, Pair<Integer, Integer>> temp = new Pair<>(coordinate,
									widthWeight);
							buttonMap.put(entry.getKey().getName(), temp);
						} else {
							buttonMap.remove(entry.getKey().getName());
							Pair<Integer, Integer> coordinate = new Pair<>(wrapper.getX() - nodeWidth / 2,
									wrapper.getY() - nodeHeight / 2);
							Pair<Integer, Integer> widthWeight = new Pair<>(nodeWidth + wrapper.getX() - nodeWidth / 2,
									nodeHeight + wrapper.getY() - nodeHeight / 2);
						...
						break;
					case "SENSOR":
						g.drawImage(imgSensor, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);
						g.drawString(nodeName, wrapper.getX() - f.stringWidth(nodeName) / 2,
								wrapper.getY() + nodeHeight);
						if (!buttonMap.containsKey(entry.getKey().getName())) {
						...
						break;
					case "ACTUATOR":
						g.drawImage(imgActuator, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);
						g.drawString(nodeName, wrapper.getX() - f.stringWidth(nodeName) / 2,
								wrapper.getY() + nodeHeight);
						if (!buttonMap.containsKey(entry.getKey().getName())) {
						...
						break;
					case "SENSOR_MODULE":
						g.drawImage(imgSensorModule, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);
						g.drawString(nodeName, wrapper.getX() - f.stringWidth(nodeName) / 2,
								wrapper.getY() + nodeHeight);
						break;
					case "ACTUATOR_MODULE":
						g.drawImage(imgActuatorModule, wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
								nodeWidth, nodeHeight, this);
						g.drawString(nodeName, wrapper.getX() - f.stringWidth(nodeName) / 2,
								wrapper.getY() + nodeHeight);
						break;
					...
					// g.fillOval(wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
					// nodeWidth, nodeHeight);
					// g.setColor(Color.black);
					// g.drawOval(wrapper.getX() - nodeWidth / 2, wrapper.getY() - nodeHeight / 2,
					// nodeWidth, nodeHeight);
					// //System.out.println((wrapper.getX())+" "+(wrapper.getY()));

					// g.drawString(entry.getKey().getName(), wrapper.getX() -
					// f.stringWidth(entry.getKey().getName()) / 2, wrapper.getY() + f.getHeight() /
					// 2);

				}

				// draw edges first
				// TODO: we draw one edge two times at the moment because we have an undirected
				// graph. But this
				// shouldn`t matter because we have the same edge costs and no one will see in.
				...
				for (Entry<Node, List<Edge>> entry : graph.getAdjacencyList().entrySet()) {
					...
					for (Edge edge : entry.getValue()) {
						/*
						 * // if other direction was drawn already continue if
						 * (drawnList.containsKey(edge.getNode()) &&
						 * drawnList.get(edge.getNode()).contains(entry.getKey())) { continue; }
						 */
						...
						// drawArrow(g, startNode.getX(), startNode.getY(), targetNode.getX(),
						// targetNode.getY());
						...
						/*
						 * int labelX = (startNode.getX() - targetNode.getX()) / 2; int labelY =
						 * (startNode.getY() - targetNode.getY()) / 2;
						 * 
						 * labelX *= -1; labelY *= -1;
						 * 
						 * labelX += startNode.getX(); labelY += startNode.getY();
						 */
						// g.setColor(Color.BLACK);
						// g.drawString(String.valueOf(edge.getInfo()), labelX -
						// f.stringWidth(String.valueOf(edge.getInfo())) / 2, labelY + f.getHeight() /
						// 2);
					}
				}
			}
	 ...
	 * //System.out.println("Drawing arrow"); double dx = x2 - x1, dy = y2 - y1;
	 * double angle = Math.atan2(dy, dx); int len = (int) Math.sqrt(dx * dx + dy *
	 * dy); AffineTransform at = AffineTransform.getTranslateInstance(x1, y1);
	 * at.concatenate(AffineTransform.getRotateInstance(angle)); g.transform(at);
	 * 
	 * // Draw horizontal arrow starting in (0, 0) QuadCurve2D.Double curve = new
	 * QuadCurve2D.Double(0,0,50+0.5*len,50,len,0); g.draw(curve); g.fillPolygon(new
	 * int[] { len, len - ARR_SIZE, len - ARR_SIZE, len }, new int[] { 0, -ARR_SIZE,
	 * ARR_SIZE, 0 }, 4); }
	 */
	 ...
	 */
	public Map<String, Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>> getButtonMap() {
		return buttonMap;
	}

	/**
	 * @param buttonMap
	 *            the buttonMap to set
	 */
	public void setButtonMap(Map<String, Pair<Pair<Integer, Integer>, Pair<Integer, Integer>>> buttonMap) {
